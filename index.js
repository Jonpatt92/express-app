...
// Add a footnote to a specific Paper
app.post('/api/v1/papers/:id', (request, response) => {
  ...
  database('papers').where('id', paper_id).select()
    .then(paper => {
      if (paper.length) {
        database('footnotes').insert({paper_id: paper_id, note: footnote['note']}, 'id')
          .then(footnote => {
